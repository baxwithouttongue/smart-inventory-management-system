...
class MinHeap:                              # Creates a MinHeap Class
    ...
    def parent(self, i):                    # Parent index
        return (i - 1) // 2                       
    ...
    # Bubble up to move a new element from the bottom of the Heap
    def insert(self, key, value):           # Defines a method to insert a new element into the MinHeap
        ...
        while i > 0:                        # Creates a loop when index 0 has not been reached
            parent_idx = (i - 1) // 2       # Calculates the parent index of the current node
    ...
    # Bubble down to remove the root element until the root is smaller than both children        
    ...
    def minHeapify(self, i, n):             # Defines a method, i is the root of the subtree, n is the current size of the heap elements
        ...
        if left < n and self.data[left][0] < self.data[smallest][0]:            # If left child exists and its priority is less than the current smallest node
            smallest = left                                                     # The left child becomes the node for swapping
        ...
        if right < n and self.data[right][0] < self.data[smallest][0]:          # If right child exists and its priority is less than the current smallest node
            smallest = right                                                    # The right child becomes the node for swapping        
        if smallest == i:                                                       # Base case, stops if no swap is needed
            return
        
        self.data[i], self.data[smallest] = self.data[smallest], self.data[i]   # Recursive steps: Swaps the parent with smaller child and moves the larger value down
        self.minHeapify(smallest, n)                                            # Calls minHeapify function recursively to continue the function until base case is met
