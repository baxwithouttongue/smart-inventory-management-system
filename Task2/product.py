...
class Product:                                                    # Product class creates a blueprint for creating product objects
    def __init__(self, data):                                     # Creates a constructor method to initialize the instance attributes of the product class
        ...
        self.avg_daily_demand = int(str(data['avg_daily_demand']))  # Stores the average daily demand as an integer
    ...
    def reorder_point(self):
        return self.safety_stock + (self.avg_daily_demand * self.lead_time)       # The formula: reorder point = safety stock + (average daily demand × lead time)
